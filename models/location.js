...
class Location {
  ...
  static async update({ loc_id, data }) {
    const locationCheck = await db.query(
      `SELECT loc_id, city, neighborhood FROM locations WHERE loc_id = $1`,
      [loc_id]
  );
  ...
  if (locationCheck.rows.length === 0) {
    ...
    // in case the string is more than one word AND
    // To account for anyone entering lowercase, will always capitalize the first letter
    const capCity = data.city ? data.city.split(' ').map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(' ') : locationCheck.rows[0].city;
    const capNeighborhood = data.neighborhood ? data.neighborhood.split(' ').map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(' ') : locationCheck.rows[0].neighborhood;
    const duplicateCheck = await db.query(
      `SELECT loc_id,
        neighborhood
       FROM locations
